...
public class chuongtrinhtimduongvuotchuongngaivat {
	...
	public static void timduongdimoi() {
		root = new O(X1, Y1);
	...
	public static void timduongdendich(O root) {
		...
		if (OF == 0)
			...
			if (vitri == 1) {
				O newO = new O(root.i - 1, root.j);
				root.trai = newO;
				timduongdendich(root.trai);
			}
			if (vitri == 2) {
				O newO = new O(root.i, root.j - 1);
				root.tren = newO;
				timduongdendich(root.tren);
			}
			if (vitri == 3) {
				O newO = new O(root.i + 1, root.j);
				root.phai = newO;
				timduongdendich(root.phai);
			}
			if (vitri == 4) {
				O newO = new O(root.i, root.j + 1);
				root.duoi = newO;
				timduongdendich(root.duoi);
			}
		}
	}
	...
	public static void docduongdi(O root) {
		...
		if (root.tren != null)
			docduongdi(root.tren);
		if (root.duoi != null)
			docduongdi(root.duoi);
		if (root.trai != null)
			docduongdi(root.trai);
		if (root.phai != null)
			docduongdi(root.phai);
	}
